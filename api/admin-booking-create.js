const { body, clean, env, escapeHtml, json, requireAdmin, supabase, tokenPair, trySendEmail } = require('./_lib');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (customer.email) { const manageUrl = `${env('PUBLIC_SITE_URL')}/booking.html?token=${encodeURIComponent(token)}`; await trySendEmail({ to: customer.email, subject: `Hair by Maeva — Booking ${booking.booking_number}`, html: `<p>Hi ${escapeHtml(fullName)},</p><p>Your booking <strong>${escapeHtml(booking.booking_number)}</strong> was created for ${escapeHtml(date)} at ${escapeHtml(time)}.</p><p><a href="${manageUrl}">View / manage my booking</a></p>` }); }
    return json(res, 201, { ok: true, bookingNumber: booking.booking_number });
  } catch (error) { console.error(error); return json(res, error.statusCode || 500, { error: error.statusCode ? error.message : 'Unable to create booking.' }); }
};
