...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (customer?.email) {
      ...
      const customerHtml = `<div style="margin:0 auto;max-width:620px;background:#f5efe6;color:#4d2c2e;font-family:Arial,sans-serif;padding:28px"><div style="background:#4d2c2e;color:#f5efe6;padding:24px;text-align:center"><div style="font-size:12px;letter-spacing:3px">HAIR BY MAEVA</div><div style="font-family:Georgia,serif;font-size:28px;margin-top:16px">Payment received</div></div><div style="background:#fffaf4;padding:28px"><p style="font-size:18px">Hi ${escapeHtml(customer.full_name)},</p><p style="line-height:1.6">We received your payment submission for booking <strong>${escapeHtml(booking.booking_number)}</strong>. Maeva will verify the payment manually. Your appointment is not confirmed until that verification is complete.</p>${details}<p style="text-align:center;margin:28px 0"><a href="${manageUrl}" style="display:inline-block;background:#4d2c2e;color:#f5efe6;padding:14px 22px;text-decoration:none">VIEW / MANAGE BOOKING</a></p></div><div style="padding:20px;text-align:center;font-size:13px;color:#765e58">With love,<br><strong style="color:#4d2c2e">Maeva</strong></div></div>`;
      ...
      if (process.env.ADMIN_EMAIL) {
        const adminHtml = `<div style="margin:0 auto;max-width:620px;background:#f5efe6;color:#4d2c2e;font-family:Arial,sans-serif;padding:28px"><div style="background:#4d2c2e;color:#f5efe6;padding:24px"><div style="font-size:12px;letter-spacing:3px">HAIR BY MAEVA · ADMIN</div><div style="font-family:Georgia,serif;font-size:28px;margin-top:16px">Payment to verify</div></div><div style="background:#fffaf4;padding:28px"><p style="font-size:18px">A client submitted a payment.</p><p style="line-height:1.6"><strong>${escapeHtml(customer.full_name)}</strong> submitted a ${escapeHtml(method.name)} payment for booking <strong>${escapeHtml(booking.booking_number)}</strong>.</p>${details}<p style="line-height:1.6"><strong>Next step:</strong> Open the Admin booking section, cross-check the payment in the payment app, then click <strong>CONFIRM</strong> if the payment was received. The client will receive the final confirmation email automatically.</p><p style="text-align:center;margin:28px 0"><a href="${env('PUBLIC_SITE_URL')}/admin.html" style="display:inline-block;background:#4d2c2e;color:#f5efe6;padding:14px 22px;text-decoration:none">OPEN ADMIN BOOKINGS</a></p></div></div>`;
