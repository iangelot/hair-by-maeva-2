...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (customer?.email) {
      ...
      if (confirmed) await trySendTemplatedEmail({ templateKey: 'payment_confirmed', to: customer.email, replyTo: process.env.ADMIN_ROUTING_EMAIL || undefined, subject, html: confirmationHtml, variables });
      else await trySendEmail({ to: customer.email, replyTo: process.env.ADMIN_ROUTING_EMAIL || undefined, subject, html: confirmationHtml });
    }
