...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (customer?.email) {
      const manageToken = decryptToken(booking.access_token_ciphertext); const manageUrl = `${env('PUBLIC_SITE_URL')}/booking?token=${encodeURIComponent(manageToken)}`; const location = '1941 West Huron Street, Chicago, IL 60622'; const paymentMethod = method?.name || 'Manual payment';
