...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const rows = await supabase(`bookings?id=eq.${encodeURIComponent(bookingId)}&select=id,booking_number,customer_id,service_name_snapshot,length_name_snapshot,appointment_date,appointment_time,total_price,reservation_fee,remaining_balance,payment_method_id,access_token_ciphertext`);
    ...
    await supabase(`payments?booking_id=eq.${encodeURIComponent(booking.id)}&status=eq.payment_submitted`, { method: 'PATCH', body: JSON.stringify({ status, verified_at: confirmed ? new Date().toISOString() : null }) });
