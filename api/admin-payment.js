const { adminRecipients, body, clean, decryptToken, env, escapeHtml, json, requireAdmin, supabase, trySendEmail, trySendTemplatedEmail } = require('./_lib');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (customer?.email) {
      ...
      const variables = { booking_number: booking.booking_number, customer_name: customer.full_name, service: booking.service_name_snapshot, length: booking.length_name_snapshot, date: booking.appointment_date, time: booking.appointment_time, total: Number(booking.total_price).toFixed(2), deposit: Number(booking.reservation_fee).toFixed(2), remaining: Number(booking.remaining_balance).toFixed(2), payment_method: method?.name || 'Manual payment', payment_status: status, manage_url: manageUrl };
      if (confirmed) await trySendTemplatedEmail({ templateKey: 'payment_confirmed', to: customer.email, replyTo: process.env.ADMIN_ROUTING_EMAIL || undefined, subject, html: `<p>Hi ${escapeHtml(customer.full_name)},</p><p>${message}</p>${details}<p>Payment method: ${escapeHtml(method?.name || 'Manual payment')}</p><p><a href="${manageUrl}">View / manage my booking</a></p>`, variables });
      else await trySendEmail({ to: customer.email, replyTo: process.env.ADMIN_ROUTING_EMAIL || undefined, subject, html: `<p>Hi ${escapeHtml(customer.full_name)},</p><p>${message}</p>${details}<p>Payment method: ${escapeHtml(method?.name || 'Manual payment')}</p><p><a href="${manageUrl}">View / manage my booking</a></p>` });
    }
