...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    await sendEmail({ to: adminRecipients(), replyTo: email, subject: `Hair by Maeva contact — ${topic}`, html: `<p><strong>${name}</strong> (${email}) sent a message.</p><p>${message.replaceAll('\n', '<br>')}</p>` });
