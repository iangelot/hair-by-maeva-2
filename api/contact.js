...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const duplicates = await supabase(`contact_messages?email=eq.${encodeURIComponent(email)}&topic=eq.${encodeURIComponent(topic)}&message=eq.${encodeURIComponent(message)}&created_at=gte.${encodeURIComponent(duplicateSince)}&select=id&limit=1`);
    if (duplicates.length) return json(res, 200, { ok: true, duplicate: true, emailSent: false });
