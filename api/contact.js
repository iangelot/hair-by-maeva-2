...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const name = clean(input.name, 120); const email = clean(input.email, 160).toLowerCase(); const topic = clean(input.topic, 80).replace(/[\r\n]/g, ' '); const message = clean(input.message, 3000);
    ...
    const emailSent = await trySendEmail({ to: adminRecipients(), replyTo: email, subject: `Hair by Maeva contact — ${topic}`, html: `<p><strong>${escapeHtml(name)}</strong> (${escapeHtml(email)}) sent a message.</p><p>${escapeHtml(message).replaceAll('\n', '<br>')}</p>` });
