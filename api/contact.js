...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const emailSent = await trySendEmail({ to: adminRecipients(), replyTo: email, subject: `Hair by Maeva contact — ${topic}`, html: `${contactDetails}<p>${escapeHtml(message).replaceAll('\n', '<br>')}</p>` });
