...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const rows = await supabase(`bookings?access_token_hash=eq.${hash}&token_expires_at=gt.${encodeURIComponent(new Date().toISOString())}&select=booking_number,service_name_snapshot,length_name_snapshot,options_snapshot,appointment_date,appointment_time,total_price,reservation_fee,remaining_balance,payment_status,status,created_at`);
