...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return json(res, 200, { policies, socials, sections, services: services.map((item) => ({ ...item, image_url: publicMediaUrl(item.image_path) })), categories, gallery: gallery.map((item) => ({ ...item, public_url: publicMediaUrl(item.image_path) })) });
  } catch (error) {
    console.error(error);
