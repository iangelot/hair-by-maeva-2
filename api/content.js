const { env, json, supabasePublic } = require('./_lib');

module.exports = async function handler(req, res) {
  if (req.method !== 'GET') return json(res, 405, { error: 'Method not allowed' });
  try {
    const [policies, socials, sections, services, gallery, categories] = await Promise.all([
      ...
      supabasePublic('services?is_active=eq.true&select=id,name,slug,description,image_path,category:service_categories(name,slug),lengths:service_lengths(name,price,display_order),options:service_options(name,price_delta,display_order,is_active)&order=display_order.asc'),
    ...
    return json(res, 200, { policies, socials, sections, services, categories, gallery: gallery.map((item) => ({ ...item, public_url: item.image_path.startsWith('http') ? item.image_path : `${mediaBase}${item.image_path}` })) });
  } catch (error) {
    console.error(error);
