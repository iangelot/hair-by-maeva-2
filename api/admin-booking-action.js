...
module.exports = async function handler(req, res) {
  ...
  } catch (error) { console.error(error); if (/overlap|already booked|existing booking/i.test(error.message || '')) return json(res, 409, { error: 'That appointment time was just taken.' }); return json(res, error.statusCode || 500, { error: error.message || 'We could not update that booking.' }); }
};
