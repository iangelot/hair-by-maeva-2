...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const booking = (await supabase(`bookings?id=eq.${encodeURIComponent(bookingId)}&select=id,booking_number,customer_id,status,appointment_date,appointment_time,duration_minutes`))[0];
    ...
    if (action === 'reschedule') {
      ...
      if (customer?.email) { const html = `<p>Hi ${escapeHtml(customer.full_name)},</p><p>Your booking <strong>${escapeHtml(booking.booking_number)}</strong> has been rescheduled to ${escapeHtml(date)} at ${escapeHtml(time)}.</p>`; await trySendEmail({ to: customer.email, replyTo: process.env.ADMIN_ROUTING_EMAIL || undefined, subject: `Hair by Maeva — Booking ${booking.booking_number} rescheduled`, html }); }
    ...
    const customer = (await supabase(`customers?id=eq.${encodeURIComponent(booking.customer_id)}&select=full_name,email`))[0];
    if (customer?.email) await trySendEmail({ to: customer.email, replyTo: process.env.ADMIN_ROUTING_EMAIL || undefined, subject: `Hair by Maeva — Booking ${booking.booking_number} cancelled`, html: `<p>Hi ${escapeHtml(customer.full_name)},</p><p>Your booking <strong>${escapeHtml(booking.booking_number)}</strong> has been cancelled by Hair by Maeva. Please contact us if you have questions.</p>` });
